...
def create_executable(specpath, src, work_path):
   ...
   pyinstaller_command = ['python', '-m', 'PyInstaller', '--onefile', '--name=dunes', src+'/src/dunes/__main__.py', '--specpath', specpath, '--noconfirm', '--workpath', work_path]
...
def create_archive_or_install(directory, tar_file, layout):
   ...
   with tempfile.TemporaryDirectory() as temp_dir:
      ...
      shutil.copy(directory+'/dist/'+exe_name, path+'/bin/dunes')
      ...
      if not install:
         if layout == 'macos' or layout == 'linux':
            ...
            with zipfile.ZipFile(tar_file, 'w', zipfile.ZIP_DEFLATED) as zf:
               for file in glob.glob(temp_d+'/**/*', recursive=True):
                  print(file)
...
if __name__ == '__main__':
   ...
   with tempfile.TemporaryDirectory() as temp_d:
      ...
      create_archive_or_install(args.src, args.tarball, args.layout)
